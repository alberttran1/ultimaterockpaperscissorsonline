...
const GameScreen = () => {
  ...
  const displayRoundResults = (data: {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          setRoundResult("");
          setMatchEndData((prev) =>
            prev ? { ...prev, showModal: true } : null,
          );
        }, 2000);
      });
    }, 1000);
  ...
  useEffect(() => {
    const handlers = {
      "rejoin-success": (data: GameInfo) => {
        ...
        if (data.gameEnded) {
          setMatchEndData({
            showModal: true,
            winner: data.winner,
            matchResult: data.winner === user.uid ? "WIN" : "LOSE",
          });
          setDisabled(true);
          setTimer(undefined);
        } else {
      ...
      "rejoin-failed": (_data: { reason: string }) => {
        setLoading(false);
      },
      ...
      "match-end": (data: {
        ...
        setMatchEndData((prev) => ({
          showModal: prev?.showModal || false,
          winner: data.winner,
  ...
  return (
    <div className="w-screen h-screen flex overflow-hidden">
        ...
        <div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            <div className="w-fit h-fit bottom-[10%] bg-[#472d7c] shadow-lg rounded-lg flex flex-col gap-4 p-10">
                ...
                {["ROCK", "PAPER", "SCISSORS"].map((hand) => {
                  ...
                  return (
                    <motion.div
                      ...
                      onClick={() => {
                        if (!disabled)
                          setHand(hand as "ROCK" | "PAPER" | "SCISSORS");
                      }}
                    >
                      <Icon />
                    </motion.div>
              ...
              <motion.div
                whileHover={
                  playingHandState !== "RANDOM" && !disabled
                    ? { scale: 1.1, opacity: 1 }
                    : {}
                }
                initial={{ opacity: 0.9 }}
      ...
      <div className=" flex-1 min-w-[40rem] h-screen relative pt-10 px-10">
              ...
              {Array.from({ length: 4 }).map((_, i) => (
                <div
                  ...
                  className={`w-4 h-4 rounded-full ${
                    i < gameInfo.score[gameInfo.players[currentUserIndex].uid]
                      ? "bg-green-400"
                      : "bg-gray-600"
                  }`}
                />
            ...
            <div className="flex gap-2">
              {Array.from({ length: 4 }).map((_, i) => (
                <div
                  ...
                  className={`w-4 h-4 rounded-full ${
                    i >=
                    4 - gameInfo.score[gameInfo.players[otherUserIndex].uid]!
                      ? "bg-green-400"
                      : "bg-gray-600"
                  }`}
                />
        ...
        <AnimatePresence>
          {showRoundStart && <RoundStart round={gameInfo.round} />}
        </AnimatePresence>
